...
class GameCUBE:
    ...
    def __init__(self, bot):
        self.players_score = {}  # playerID - score
        self.players_name = {}  # playerID - name
        self.bot = bot
        self.keyboard = None

    def game(self, num, playerID):
        choice = int(num)
        msg = self.bot.send_dice(playerID, emoji=u'\U0001F3B2')
        self.bot.send_message(playerID, f'Вам выпало {msg.dice.value}')

        if choice == msg.dice.value:
            self.bot.send_message(playerID, "Вы проиграли(", reply_markup=self.keyboard)
        else:
            self.players_score[playerID] += abs(choice - msg.dice.value)
            self.bot.send_message(playerID, f"Пронесло...Вас счет {self.players_name[playerID]}: "
                                            f"{self.players_score[playerID]}", reply_markup=self.keyboard)

    def start(self, playerID, playerName):
        print("cube start")
        self.players_score[playerID] = 0
        self.players_name[playerID] = playerName
        self.input_1(playerID)
        keyboard = types.InlineKeyboardMarkup()
        list_btn = []
        for keyName in self.buttons:
            list_btn.append(types.InlineKeyboardButton(text=keyName,
                                                       callback_data="GameCUBE|Choice-" + keyName + "|" + menuBot.Menu.setExtPar(
                                                           self)))
        keyboard.add(*list_btn)
        list_btn = types.InlineKeyboardButton(text="Выход",
                                              callback_data="GameCUBE|Exit|" + menuBot.Menu.setExtPar(self))
        keyboard.add(list_btn)
        self.keyboard = keyboard
        print(keyboard)
        print("cube start_end")

    def input_1(self, playerID):
        print("cube input_1")
        mes = self.bot.send_message(playerID, text='Введите число от 1-6', reply_markup=types.ReplyKeyboardRemove())
        self.bot.register_next_step_handler(mes, self.input_2, playerID)
        print("cube input_1_end")

    def input_2(self, mes, playerID):
        print("cube input_2")
        num = mes.text
        res = self.catch_error(num)
        print("res ", res)
        if res is True:
            self.game(num, playerID)
        else:
            self.bot.send_message(playerID, text="Вы ввели не цифру от 1 до 6", reply_markup=self.keyboard)
        print("cube input_2_end")

    def catch_error(self, input):
        print("cube catcherror")
        if input in ["1", "2", "3", "4", "5", "6"]:
            try:
                return isinstance(int(input), int)
            except KeyError:
                return False
        else:
            return False
    ...
    def delPlayer(self, playerID):
        self.players_score.pop(playerID)
        self.players_name.pop(playerID)
        stopGame(playerID)

    def CheckGlobalScore(self, playerID):
        board_score = {self.players_name[k]: self.players_score[k] for k in
                       sorted(self.players_score, key=self.players_score.get, reverse=True)}
        return self.bot.send_message(playerID,
                                     f"Таблица счета всех игроков\n{'&#92;n'.join([str(k) + ' ' + str(v) for k, v in board_score.items()])}\n...")
